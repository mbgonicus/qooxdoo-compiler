...
module.exports = qx.Class.define("qx.tool.compiler.Analyser", {
  ...
  members: {
    ...
    saveDatabase: function (cb) {
      ...
      fs.writeFile(this.getDbFilename(), JSON.stringify(t.__db, null, 2), function (err) {
        if (err)
          return cb(err);
        if (t.__resManager)
    ...
    getClassInfo: function(className, forceScan, cb) {
      ...
      function scanFile(stat, outputStat) {
        ...
        classFile.load(function (err) {
          ...
          // Next!
          return cb && cb(null, dbClassInfo, classFile);
        });
      }
    },
    ...
    getCldr: async function(locale) {
      ...
      if (cldr)
        return cldr;
      return qx.tool.compiler.app.Cldr.loadCLDR(t.getQooxdooPath(), locale)
        .then((cldr) => {
                  ...
                  dbClassInfo.translations.forEach(function(src) {
                    ...
                    if (qx.lang.Type.isArray(src.lineNo)) {
                      src.lineNo.forEach(function(lineNo) {
                        if (!ref.hasOwnProperty(fileName)) {
                          ref[fileName] = [];
                        }
